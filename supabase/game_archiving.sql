...
begin
  ...
  if v_target_game.archived_at is not null then
    return query select false, v_booking.id, v_booking.game_id, p_target_game_id, 'target_game_archived'::text, false, false;
    return;
  end if;
